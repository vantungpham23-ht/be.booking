...
BEGIN
    ...
    -- If booking for today, start from next available slot (30 min from now)
    IF p_date = CURRENT_DATE
       AND v_slot_start < v_current_time + INTERVAL '30 minutes'
    THEN
        v_slot_start := v_current_time + INTERVAL '30 minutes';
        v_slot_start := DATE_TRUNC('minute', v_slot_start)
                      + (FLOOR(EXTRACT(MINUTE FROM v_slot_start)::INTEGER / 15)
                         * INTERVAL '15 minutes');
    END IF;
